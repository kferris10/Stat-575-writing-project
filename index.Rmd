...
For this analysis, the null hypothesis is that the density surfaces in the two situations are the same, and the alternative is that they are different. Under the null hypothesis, pitch location is not related to whether or not pitchers are responsible for the men on base.  Therefore, under the null hypothesis, the labels are arbitrary and have no meaning.  

In this setting, permutation testing can be used to help test these hpotheses.  Because the labels are arbitrarily assigned under the null, they can be permuted (randomly reassigned) to form a permuted dataset which is just as likely have occurred under the null hypothesis.  


Explain how to test using pemutations.
